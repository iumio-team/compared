...
/**
 * Welcome on COMPARED Quieres FILE
...
class Queries {
     ...
     * @return PDOStatement Query result
     */
    static public function countLine($table, $idName) {
    ...
    static public function findRecentComparison() {
        ...
        $count -= 2;
        $query2 = "SELECT * from COMPARED order by idCOMPARED desc limit 0, $count ;";
     ...
     * @param String $object object message
     * @param String $content content message
     * @param Date $date Date of message
     * @return PDOStatement Query result
     */
    static public function updateHelpMessage($id, $name, $email, $object, $content,$date) {
        $query = "update HelpMessage name= ?, email=?,object=?,content=?,date=?  where id = ?;";
        $prepare = Connector::prepare($query, array($name, $email, $object, $content,$date, $id));
        return $prepare;
    }
     ...
     * @return PDOStatement Query result
     */
    static public function insertHelpMessage($name, $email, $object, $content,$date) {
        $query = "insert into HelpMessage (id,name,email,object,content,date) values ('',?,?,?,?,?);";
        $prepare = Connector::prepare($query, array($name, $email, $object, $content,$date));
        return $prepare;
    }
