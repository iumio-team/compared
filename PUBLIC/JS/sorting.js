		...
		jQuery('.btn_load_more').click(function(){
			...
			if (now_load > 0) {
				if (addon_options.type == 0) {
					...
					for (i = i_start-1; i < i_start+now_load-1; i++) {
						loaded_object = loaded_object + '<div class="element col-sm-4  gall '+ addon_options.items[i].category +'"><a class="plS" href="'+ addon_options.items[i].src +'" ><img class="img-responsive picsGall" src="'+ addon_options.items[i].src +'" alt="pic2 Gallery"/></a><div class="view project_descr "><h3><a href="#">'+ addon_options.items[i].title +'</a></h3><ul><li><i class="fa fa-comment"></i>'+ addon_options.items[i].lika_count +'</li></ul></div></div>';
					}
				}
